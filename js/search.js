...
async function handleSearch(event) {
    ...
    if (currentPath.includes('/Articles/') || currentPath.includes('/AboutMe/')) {
        ...
        const levelsUp = pathSegments.length - 1; // 减去根目录本身
...
async function searchInContent(articles, query) {
    ...
    for (const article of articles) {
        try {
            // 构建文章文件的URL
            const articleUrl = `Articles/${article.category}/${article.filename}`;
        ...
        if (resultList) {
            if (results.length > 0) {
                resultList.innerHTML = results.map(article => `
                    <li>
                        <a class="article-list-title" href="${article.url}">${article.title}</a>
